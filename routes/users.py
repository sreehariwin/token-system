...
@router.get('/debug/project-mismatch-check')
async def check_project_mismatch():
    """Check for Firebase project configuration mismatch"""
    import json
    ...
    try:
        ...
        if firebase_service_account:
            service_account_info = json.loads(firebase_service_account)
            result["env_project_id"] = service_account_info.get('project_id')
        
        # Get project ID from initialized Firebase app
        try:
            app = firebase_admin.get_app()
            result["firebase_initialized"] = True
            result["server_project_id"] = getattr(app, 'project_id', 'Unknown')
        except ValueError:
            result["firebase_initialized"] = False
            result["instructions"].append("Firebase not initialized - check FIREBASE_SERVICE_ACCOUNT")
        
        # Check for mismatch
        if result["env_project_id"] and result["server_project_id"]:
            if result["env_project_id"] != result["server_project_id"]:
                result["mismatch_detected"] = True
                result["instructions"].append("PROJECT MISMATCH DETECTED!")
        
        # Add instructions based on findings
        if result["env_project_id"]:
            result["instructions"].append(f"1. Your server is configured for project: {result['env_project_id']}")
            result["instructions"].append(f"2. Make sure your Flutter app's google-services.json has the same project_id")
            result["instructions"].append(f"3. Make sure the FCM token was generated from an app using project: {result['env_project_id']}")
        else:
            result["instructions"].append("FIREBASE_SERVICE_ACCOUNT not found or invalid")
...
@router.post('/debug/validate-token-project')
async def validate_token_project(console_token: str):
    """Validate token against current Firebase project with detailed logging"""
    from firebase_admin import messaging
    import firebase_admin
    
    print("🔍 VALIDATING TOKEN AGAINST PROJECT")
    print("=" * 50)
    ...
    try:
        ...
        print(f"📋 Server project ID: {getattr(app, 'project_id', 'Unknown')}")
    except ValueError as e:
        print(f"❌ Firebase not initialized: {e}")
        return {"success": False, "error": "Firebase not initialized"}
    
    # Test the token with minimal message
    try:
        print(f"📱 Testing token: {console_token[:30]}...{console_token[-10:]}")
        
        # Create the simplest possible message
        message = messaging.Message(
            data={"test": "validation"},  # Use data instead of notification for testing
            token=console_token
        )
        
        print(f"📤 Attempting to send test message...")
        ...
        return {
            "success": True, 
            "response": response,
            "message": "Token is valid for this Firebase project"
        }
        ...
        if "registration token" in error_msg:
            print("🚨 This suggests:")
            print("   - Token format is invalid, OR")
            print("   - Token was generated for a different Firebase project")
            print("   - Token is from a different app/bundle ID")
        
        return {
            "success": False,
            "error": str(e),
            "error_type": "InvalidArgumentError",
            "likely_cause": "Token was generated for a different Firebase project"
        }
        
    except messaging.UnregisteredError as e:
        print(f"❌ Unregistered error: {e}")
        return {
            "success": False,
            "error": str(e),
            "error_type": "UnregisteredError",
            "likely_cause": "App was uninstalled or token expired"
        }
        
    except messaging.SenderIdMismatchError as e:
        print(f"❌ Sender ID mismatch: {e}")
        print("🚨 DEFINITE PROJECT MISMATCH!")
        print("   Your server project doesn't match the app's project")
        
        return {
            "success": False,
            "error": str(e),
            "error_type": "SenderIdMismatchError",
            "likely_cause": "Server Firebase project doesn't match app's Firebase project"
        }
